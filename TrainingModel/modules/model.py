...
class SpeakerNet(nn.Module):
    def __init__(self, save_path, model, optimizer, callbacks, criterion, device, max_epoch, features, **kwargs):
        ...
        if self.callback in ['steplr', 'cosinelr', 'cycliclr']:
            ...
            self.__scheduler__, self.lr_step = Scheduler(
                self.__optimizer__, **dict(kwargs, T_max=self.T_max))
            ...
            self.__scheduler__ = Scheduler(
                self.__optimizer__, patience=self.scheduler_step, min_lr=1e-8, factor=0.95)
    ...
    def fit(self, loader, epoch=0):
        ...
        loader_bar = tqdm(
            loader, desc=f">EPOCH_{epoch}", unit="it", colour="green")
        ...
        for idx, filename in enumerate(tqdm(setfiles, desc=">>>>Reading file: ", unit="files", colour="red")):
            ...
            with torch.no_grad():
                ...
                ref_feat = self.__S__.forward(
                    inp1.to(self.device)).detach().cpu()
            ...
            if cohorts_path is None:
                ...
                if scoring_mode == 'norm':
                    ...
                    score = similarity_measure(
                        'cosine', ref_feat.cpu(), com_feat.cpu())
    ...
    def prepare(self,
        ...
        if prepare_type == 'cohorts':
            ...
            feats = []
            assert isinstance(
                source, str), "Please provide path to test directory"
